...
if __name__ == "__main__":
    ...
    print("Testing FocusService...")
    
    # Menampung event hasil emit
    emittedSessions = []
    emittedPomodoros = []
    
    def onFocusSessionSaved(sessionData):
        print(f"Event Received! Focus Session Saved: {sessionData['sessionType']} ({sessionData['duration']} mins)")
        emittedSessions.append(sessionData)
        
    def onPomodoroFinished(sessionData):
        print(f"Event Received! Pomodoro Completed: {sessionData['duration']} mins completed successfully!")
        emittedPomodoros.append(sessionData)
        
    # Subscribe ke event bus
    eventBus.subscribe("focusSessionSaved", onFocusSessionSaved)
    eventBus.subscribe("pomodoroFinished", onPomodoroFinished)
    
    # Buat sesi baru untuk pengujian
    testSession = saveFocusSession(
    ...
    # Ambil riwayat sesi
    history = getFocusHistory(limit=5)
    print(f"\nFocus History (Total: {len(history)}):")
    for s in history:
        print(f"- {s['sessionType']} | Duration: {s['duration']} mins | Completed: {s['completed']}")
        
    # Verifikasi fungsionalitas
    if testSession and len(emittedSessions) == 1 and len(emittedPomodoros) == 1:
        print("\nFocusService tests passed successfully!")
    else:
        print("\nFocusService tests failed.")
        exit(1)
